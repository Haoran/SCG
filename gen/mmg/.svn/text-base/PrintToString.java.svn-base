...
public class PrintToString extends edu.neu.ccs.demeterf.FC{
   ...
   public _LT combine(MMGInstance _h_, _LT x){
      return empty.append(x).compress();
   }
   public _LT combine(MMGSolution _h_, _LT y){
      return empty.append(y).compress();
   }
   public _LT combine(MMGInstanceSet _h_){
      return empty;
   }
   public _LT combine(MMGDomain _h_){
      return empty;
   }
   public _LT combine(MMGConfig _h_){
      return empty.append(new _S("mmg_config[")).append(new _S("]")).compress();
   }
